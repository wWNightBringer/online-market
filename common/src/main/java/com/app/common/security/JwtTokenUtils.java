...
public class JwtTokenUtils {
    ...
    private static Jwt buildToken(String email, String name, String role) {
        ...
        return new SecurityJwtConfiguration().jwtEncoder().encode(parameters);
    }

    private static void buildClaims(Map<String, Object> claims, String email, String name, String role) {
        claims.put(EMAIL_PARAM.name(), email);
        claims.put(ROLE_PARAM.name(), role);
        claims.put(JwtClaimNames.SUB, name);
    }
}
